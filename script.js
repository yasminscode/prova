const muscles = [
    ...
    },
    {
        ...
        innervation: "Ramos da margem da mandíbula.",
    ...
    },
    {
        ...
        origin: "Região temporal e occipital.",
        insertion: "Aponeurose epicrânica.",
        innervation: "Ramos temporais do Nervo Facial (NC VII).",
    ...
    },
    {
        ...
        origin: "Cabeça esternal e cabeça clavicular",
    ...
    },
    {
        ...
        insertion: "Músculos adjacentes, como o occiptofrontal.",
...
function createQuestionObject(correctMuscle, prop1, prop2) {
    ...
    // 2. Cria 3 alternativas incorretas 
    // Pegamos dados de outros músculos aleatórios, MAS mantemos o nome do músculo CORRETO
    const otherMuscles = muscles.filter(m => m.id !== correctMuscle.id).sort(() => Math.random() - 0.5);
    
    for (let i = 0; i < 3; i++) {
        let wrongMuscleData = otherMuscles[i];
        options.push({
            // AQUI ESTÁ A MÁGICA: Usamos o nome do músculo correto (correctMuscle.name), 
            // mas as propriedades vêm do músculo errado (wrongMuscleData)
            text: `<strong>${correctMuscle.name}</strong><br>• ${propertyLabels[prop1]}: ${wrongMuscleData[prop1]}<br>• ${propertyLabels[prop2]}: ${wrongMuscleData[prop2]}`,
            isCorrect: false
        });
    }
